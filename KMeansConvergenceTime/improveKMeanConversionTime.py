...
#Sorting data based on distance from chicgao

DistanceFromChicago = DistanceFromChicago.sort_values(['DrivingMilesFromChicago'])
trainData = numpy.reshape(numpy.asarray(DistanceFromChicago['DrivingMilesFromChicago']), (nCity, 1))
...
for kc in range(1,60):
    ...
    i = 1/(2*KClusters)
    while i < 1:
        initialGuess.append(trainData[int(i*len(trainData))])
        i += (1/KClusters)
